...
export default function InventoryList({ channelId, userId, operator = false }: { channelId?: string; userId?: string; operator?: boolean }) {
  ...
  const canAttempt = (item: InventoryItem) => {
    ...
    if (!operator && item.latest_attempt_outcome_kind === "buyer_reverted") return false;
  ...
  const canRefund = (item: InventoryItem) => item.redemption_status === "PENDING" && (operator || item.latest_attempt_outcome_kind !== "buyer_reverted") && ["WAITING_VIEWER", "WAITING_OPERATOR", "TRADE_LINK_REQUIRED", "RETRY_AVAILABLE", "INSUFFICIENT_FUNDS", "OPERATOR_REVIEW"].includes(item.lifecycle_status) && item.fulfillment_mode !== "LEGACY_REVIEW";
  return <section aria-label={c("Inventory items", "Предметы инвентаря")} className="space-y-4">
    ...
    {!query.data.length && offset === 0 ? <EmptyState title={c("No matching inventory items", "Подходящих предметов нет")} description={c("Items appear here when a reward resolves a concrete skin.", "Предмет появится здесь после выбора конкретного скина для награды.")} /> : <ul className="inventory-grid">{query.data.map(item => <li key={item.id} className="inventory-item">
      ...
      <div className="min-w-0 space-y-2">
        ...
        {item.latest_attempt_outcome_kind === "buyer_reverted" && !operator && item.redemption_status === "PENDING" && <p className="text-xs text-muted-foreground">{c("You cannot retry delivery or return Channel Points after reverting an accepted trade. Contact the channel operator for help.", "После отмены принятого обмена нельзя повторить доставку или самостоятельно вернуть баллы. Обратитесь к оператору канала.")}</p>}
        {item.latest_attempt_status === "BUYER_FAILED" && item.latest_attempt_outcome_kind !== "buyer_reverted" && !item.buyer_retry_allowed && !operator && <p className="text-xs text-muted-foreground">{c("Viewer retry is disabled for this reward.", "Повторная попытка зрителем отключена для этой награды.")}</p>}
        {item.lifecycle_status === "OPERATOR_REVIEW" && item.latest_attempt_status === "TERMINAL_UNCLASSIFIED" && item.latest_market_stage === "5" && item.redemption_status === "PENDING" && item.fulfillment_mode !== "LEGACY_REVIEW" && !operator && <p className="text-xs text-muted-foreground">{c("Market confirmed that this attempt ended, but its cause is unclear. You can return your points or wait for operator review.", "Маркет подтвердил завершение попытки, но причина неясна. Можно вернуть баллы или дождаться проверки оператора.")}</p>}
        {item.lifecycle_status === "OPERATOR_REVIEW" && item.redemption_status === "COMPLETED" && !operator && <p className="text-xs text-muted-foreground">{c("This redemption is already marked completed. Points cannot be returned here; contact the channel operator.", "Эта активация уже отмечена выполненной. Здесь нельзя вернуть баллы; обратитесь к оператору канала.")}</p>}
