...
const now = "2026-09-13T12:20:00Z";
...
export async function mockApi(page: Page, options: { role?: string; tour?: boolean; empty?: boolean; fail?: string; guest?: boolean } = {}) {
  ...
  await page.route("**/api/**", async route => {
    ...
    if (path.endsWith("/redemptions")) {
      const filtered = rows.filter(r => (!url.searchParams.get("status") || r.status === url.searchParams.get("status")) && (!url.searchParams.get("user_id") || r.user_id === url.searchParams.get("user_id")));
