...
for (const width of [1440, 390]) test(`durable Market transaction states at ${width}px`, async ({ page }) => {
  ...
  const cards = [
    ...
    { ...item("RETRY_AVAILABLE", 9), latest_attempt_status: "SELLER_FAILED", latest_attempt_outcome_kind: "seller_not_sent", latest_market_stage: "5" },
  ];
  ...
  await expect(page.locator(".inventory-item").nth(8).getByText("An earlier accepted trade was reverted by you.", { exact: false })).toBeVisible();
  await expect(page.locator(".inventory-item").nth(9).getByRole("button", { name: "Try delivery again" })).toBeVisible();
  await expect(page.locator(".inventory-item").nth(9).getByRole("button", { name: "Return Channel Points" })).toBeVisible();
  expect(await page.evaluate(() => document.documentElement.scrollWidth)).toBeLessThanOrEqual(width + 1);
